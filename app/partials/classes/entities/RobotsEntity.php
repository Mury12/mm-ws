...
use MMWS\Model\Robots;
use MMWS\Controller\RobotsController;
...
class RobotsEntity
{
    ...
    public function __construct(Robots $model)
    {
        $this->model = $model;
    }
    ...
    {
        /**
         * @var Robots $hasRobots;
         */
        $hasRobots = $this->getOne(['*'], true);
        if ($hasRobots) {
            $fields = $this->model->toArray();
            $stmt = new PDOQueryBuilder($this->model->table);
            $stmt->update($fields);
    ...
    private function getOne(array $fields = [], bool $asobj = false)
    {
        ...
        try {
            ...
            if (sizeof($instance))
                return $asobj
                    ? (new RobotsController($instance[0]))->model
                    : $instance[0];
     ...
     * @param array $fields fields to return.
     * @param array $asobj returns as Robots Object
     * @param bool $and sets if the filter sould be put with AND or OR

     * @return array|Array<Robots>
     */
        ...
        try {
            ...
            if ($asobj) {
                return array_map(function ($instance) {
                    return (new RobotsController($instance))->model;
                }, $instances);
