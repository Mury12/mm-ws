...
use MMWS\Model\RobotOwners;
use MMWS\Controller\RobotOwnersController;
use MMWS\Handler\PDOQueryBuilder;
use MMWS\Factory\RequestExceptionFactory;
use PDOException;

class RobotOwnersEntity
{
    
    public $model;

    public function __construct(RobotOwners $model)
    {
        $this->model = $model;
    }
    ...
    {
        /**
         * @var RobotOwners $hasRobotOwners;
         */
        $hasRobotOwners = $this->getOne(['*'], true);
        if ($hasRobotOwners) {
            $fields = $this->model->toArray();
            $stmt = new PDOQueryBuilder($this->model->table);
            $stmt->update($fields);
    ...
    private function getOne(array $fields = [], bool $asobj = false)
    {
        ...
        try {
            ...
            if (sizeof($instance))
                return $asobj
                    ? (new RobotOwnersController($instance[0]))->model
                    : $instance[0];
     ...
     * @param array $fields fields to return.
     * @param array $asobj returns as RobotOwners Object
     * @param bool $and sets if the filter sould be put with AND or OR

     * @return array|Array<RobotOwners>
     */
        ...
        try {
            ...
            if ($asobj) {
                return array_map(function ($instance) {
                    return (new RobotOwnersController($instance))->model;
                }, $instances);
