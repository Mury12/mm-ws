...
use MMWS\Controller\RobotAuthController;
use MMWS\Handler\PDOQueryBuilder;
use MMWS\Factory\RequestExceptionFactory;
use PDOException;

class RobotAuthEntity
{
    
    public $model;

    public function __construct(RobotAuth $model)
    {
        $this->model = $model;
    }
    ...
    {
        /**
         * @var RobotAuth $hasRobotAuth;
         */
        $hasRobotAuth = $this->getOne(['*'], true);
        if ($hasRobotAuth) {
            $fields = $this->model->toArray();
            $stmt = new PDOQueryBuilder($this->model->table);
            $stmt->update($fields);
    ...
    private function getOne(array $fields = [], bool $asobj = false)
    {
        ...
        try {
            ...
            if (sizeof($instance))
                return $asobj
                    ? (new RobotAuthController($instance[0]))->model
                    : $instance[0];
     ...
     * @param array $fields fields to return.
     * @param array $asobj returns as RobotAuth Object
     * @param bool $and sets if the filter sould be put with AND or OR

     * @return array|Array<RobotAuth>
     */
        ...
        try {
            ...
            if ($asobj) {
                return array_map(function ($instance) {
                    return (new RobotAuthController($instance))->model;
                }, $instances);
