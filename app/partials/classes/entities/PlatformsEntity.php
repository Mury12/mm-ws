...
use MMWS\Controller\PlatformsController;
...
class PlatformsEntity
{
    ...
    public function __construct(Platforms $model)
    {
        $this->model = $model;
    }
    ...
    {
        /**
         * @var Platforms $hasPlatforms;
         */
        $hasPlatforms = $this->getOne(['*'], true);
        if ($hasPlatforms) {
            $fields = $this->model->toArray();
            $stmt = new PDOQueryBuilder($this->model->table);
            $stmt->update($fields);
    ...
    private function getOne(array $fields = [], bool $asobj = false)
    {
        ...
        try {
            ...
            if (sizeof($instance))
                return $asobj
                    ? (new PlatformsController($instance[0]))->model
                    : $instance[0];
     ...
     * @param array $fields fields to return.
     * @param array $asobj returns as Platforms Object
     * @param bool $and sets if the filter sould be put with AND or OR

     * @return array|Array<Platforms>
     */
        ...
        try {
            ...
            if ($asobj) {
                return array_map(function ($instance) {
                    return (new PlatformsController($instance))->model;
                }, $instances);
