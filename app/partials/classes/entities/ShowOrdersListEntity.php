...
use MMWS\Model\ShowOrdersList;
use MMWS\Controller\ShowOrdersListController;
use MMWS\Handler\PDOQueryBuilder;
use MMWS\Factory\RequestExceptionFactory;
use PDOException;

class ShowOrdersListEntity
{
    
    public $model;

    public function __construct(ShowOrdersList $model)
    {
        $this->model = $model;
    }
    ...
    {
        /**
         * @var ShowOrdersList $hasShowOrdersList;
         */
        $hasShowOrdersList = $this->getOne(['*'], true);
        if ($hasShowOrdersList) {
            $fields = $this->model->toArray();
            $stmt = new PDOQueryBuilder($this->model->table);
            $stmt->update($fields);
    ...
    private function getOne(array $fields = [], bool $asobj = false)
    {
        ...
        try {
            ...
            if (sizeof($instance))
                return $asobj
                    ? (new ShowOrdersListController($instance[0]))->model
                    : $instance[0];
     ...
     * @param array $fields fields to return.
     * @param array $asobj returns as ShowOrdersList Object
     * @param bool $and sets if the filter sould be put with AND or OR

     * @return array|Array<ShowOrdersList>
     */
        ...
        try {
            ...
            if ($asobj) {
                return array_map(function ($instance) {
                    return (new ShowOrdersListController($instance))->model;
                }, $instances);
