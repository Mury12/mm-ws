...
use MMWS\Model\RobotOrders;
use MMWS\Controller\RobotOrdersController;
use MMWS\Handler\PDOQueryBuilder;
use MMWS\Factory\RequestExceptionFactory;
use PDOException;

class RobotOrdersEntity
{
    
    public $model;

    public function __construct(RobotOrders $model)
    {
        $this->model = $model;
    }
    ...
    {
        /**
         * @var RobotOrders $hasRobotOrders;
         */
        $hasRobotOrders = $this->getOne(['*'], true);
        if ($hasRobotOrders) {
            $fields = $this->model->toArray();
            $stmt = new PDOQueryBuilder($this->model->table);
            $stmt->update($fields);
    ...
    private function getOne(array $fields = [], bool $asobj = false)
    {
        ...
        try {
            ...
            if (sizeof($instance))
                return $asobj
                    ? (new RobotOrdersController($instance[0]))->model
                    : $instance[0];
     ...
     * @param array $fields fields to return.
     * @param array $asobj returns as RobotOrders Object
     * @param bool $and sets if the filter sould be put with AND or OR

     * @return array|Array<RobotOrders>
     */
        ...
        try {
            ...
            if ($asobj) {
                return array_map(function ($instance) {
                    return (new RobotOrdersController($instance))->model;
                }, $instances);
